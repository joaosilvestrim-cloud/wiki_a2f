...
const DocumentsPage = () => {
  ...
  const fetchDocuments = async () => {
    ...
    // CORREÇÃO DE SEGURANÇA: Apenas admins podem ver todos os documentos
    let query = supabase
      .from('documents')
      .select(`
        *,
    ...
    // Usuários não-admin só veem documentos públicos/compartilhados
    if (!user.is_admin) {
      // Por ora, documentos da empresa são visíveis para todos os usuários autenticados
      // mas a implementação pode ser refinada conforme necessidade
    }
  ...
  const getFileColor = (type) => {
    ...
    if (type?.includes('word')) return 'text-blue-400 bg-blue-500/20';
    if (type?.includes('presentation')) return 'text-orange-400 bg-orange-500/20';
    if (type?.includes('sheet')) return 'text-green-400 bg-green-500/20';
    ...
    return (
      <div className="flex justify-center items-center py-12">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Acesso não autorizado</h3>
          <p className="text-purple-200">Por favor, faça login para acessar esta página.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Documentos da Empresa</h1>
          <p className="text-purple-200 mt-1">Documentos compartilhados com todos os funcionários</p>
        </div>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" />
        {user.is_admin && (
          <Button onClick={handleUploadClick} disabled={uploading} className="mt-4 sm:mt-0 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Buscar documentos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
          </div>
          <div className="flex gap-2">
            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
              {categories.map(category => <option key={category} value={category} className="bg-slate-800">{category}</option>)}
            </select>
            <Button variant="outline" onClick={() => handleAction('Filtros Avançados')} className="border-white/20 text-white hover:bg-white/10">
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDocuments.map((document, index) => (
              <motion.div key={document.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.05 }} className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 hover:bg-white/20 transition-all group">
                  ...
                  {user.is_admin && (
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button variant="ghost" size="icon" className="text-purple-300 hover:text-red-500 hover:bg-red-500/10">
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => handleDelete(document)}>Excluir</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )}
                ...
                <div className="mb-4">
                  <h3 className="text-white font-semibold mb-2 line-clamp-2">{document.name}</h3>
                  <div className="flex items-center justify-between text-sm text-purple-200">
                    <span className="px-2 py-1 bg-purple-500/20 rounded text-purple-300 uppercase">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    <img src={document.profile?.avatar_url || `https://ui-avatars.com/api/?name=${document.profile?.name || '?'}&background=random`} alt={document.profile?.name} className="w-6 h-6 rounded-full border border-purple-400" />
                    <span className="text-sm text-purple-200">{document.profile?.name || 'Desconhecido'}</span>
                  </div>
                </div>
                <div className="flex space-x-2">
                  <Button size="sm" onClick={() => handleDownload(document.file_path, document.name, document.id)} className="flex-1 bg-gradient-to-r from-purple-500/20 to-indigo-500/20 border border-purple-400/30 text-white hover:border-purple-400/50" variant="outline">
                    <Download className="w-4 h-4 mr-1" />
                    Download
                  </Button>
                  <Button size="sm" onClick={() => handleView(document)} className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600">
      ...
      <Dialog open={!!viewingDoc} onOpenChange={() => setViewingDoc(null)}>
        <DialogContent className="max-w-4xl h-[90vh] bg-card border-border text-foreground flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>Documento da empresa</DialogDescription>
          </DialogHeader>
